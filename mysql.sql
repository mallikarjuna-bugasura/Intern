...
SELECT w.post
FROM tUser u1
LEFT OUTER JOIN tFriends f ON u1.user_id = f.user_id
LEFT OUTER JOIN tUser u2 ON f.friend_id = u2.user_id
LEFT OUTER JOIN tWall w ON w.user_id = u2.user_id
...
LEFT OUTER JOIN tFriends f ON u1.user_id = f.user_id
LEFT OUTER JOIN tUser u2 ON f.friend_id = u2.user_id
LEFT OUTER JOIN tFriends ff ON ff.user_id = u2.user_id
LEFT OUTER JOIN tUser u3 ON u3.user_id = ff.friend_id
WHERE u1.name = 'malli' AND u2.name = 'raja';

-- Query to remove a particular friend from a persons list, given the persons name
DELETE f
FROM tFriends f
LEFT OUTER JOIN tUser u1 ON f.user_id = u1.user_id
LEFT OUTER JOIN tUser u2 ON f.friend_id = u2.user_id
